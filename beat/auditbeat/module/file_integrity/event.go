...
func buildMetricbeatEvent(e *Event, existedBefore bool) mb.Event {
	...
	if len(e.LookupResults) > 0 {
		out.MetricSetFields.Put("intelix", e.LookupResults)
	}
...
func (e *Event) AddLookupResult(key string, result interface{}) {
	if e.LookupResults == nil {
		e.LookupResults = make(map[string]interface{})
	}

	e.LookupResults[key] = result
}
