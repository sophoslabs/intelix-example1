...
func New(base mb.BaseMetricSet) (mb.MetricSet, error) {
	...
	i, err := NewIntelix(&config.Intelix)
	if err != nil {
		return nil, errors.Wrap(err, "failed to initialize Intelix")
	}
...
func (ms *MetricSet) Lookup(event *Event) {
	// TODO: run lookups in a separate goroutine, feed with chan
	changed, lastEvent := ms.hasFileChangedSinceLastEvent(event)
	if !changed {
		return
	}

	ms.log.Infof("Intelix lookup, action '%v', file: '%v'", event.Action, event.Path)

	if !ActionTriggersAnalysis(event.Action) {
		ms.log.Infof("Action: %v, no analysis needed for file %v", event.Action, event.Path)
		return
	}

	if event.Info == nil || event.Info.Type != FileType {
		return
	}

	if event.Info.Size == 0 {
		ms.log.Infof("Size: %v, no analysis needed for file %v", event.Info.Size, event.Path)
		return
	}

	if event.Hashes != nil {
		if sha256, ok := event.Hashes[SHA256]; ok {
			if lastEvent != nil && lastEvent.Hashes != nil {
				if old_sha256, ok := lastEvent.Hashes[SHA256]; ok && sha256.String() == old_sha256.String() {
					ms.log.Infof("Sha256 did not change, no analysis needed for file %v", event.Path)
					return
				}
			}

			result, err := ms.intelix.MalwareLookup(sha256)
			if err != nil {
				event.errors = append(event.errors, err)
			}

			event.AddLookupResult("malware", result)
			// ms.log.Infof("Malware lookup result: %+v", result)

			if result.ReputationScore >= ms.config.Intelix.StaticAnalysisReputationThreshold {
				ms.log.Info("Reputation of '%v' is above threshold: %v>%v", event.Path, result.ReputationScore, ms.config.Intelix.StaticAnalysisReputationThreshold)
				return
			}
		}
	}

	staticResult, err := ms.intelix.Analysis(STATIC, event.Info.Size, event.Path)
	if err != nil {
		event.errors = append(event.errors, err)
	}

	if staticResult != nil {
		event.AddLookupResult("static", staticResult)
		//ms.log.Infof("Static analysis result: %+v", staticResult)
	}

	if staticResult.JobStatus == SUCCESS {
		if staticResult.Report.Score >= ms.config.Intelix.DynamicAnalysisScoreThreshold {
			ms.log.Infof("Static analysis score of '%v' is above threshold: %v>%v", event.Path, staticResult.Report.Score, ms.config.Intelix.DynamicAnalysisScoreThreshold)
			return
		}
	}

	dynamicResult, err := ms.intelix.Analysis(DYNAMIC, event.Info.Size, event.Path)
	if err != nil {
		event.errors = append(event.errors, err)
	}

	if dynamicResult != nil {
		event.AddLookupResult("dynamic", dynamicResult)
		//ms.log.Infof("Dynamic analysis result: %+v", dynamicResult)
	}
}
